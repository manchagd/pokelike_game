  ...
  module Battles
    class TurnResolverService
      ...
      def assign_actions_by_priority(actions)
        # actions: [{action: "attack", player_id: 1, id: 801, pokemon_id: 42, targets: ["B1"]}, {action: "switch", player_id: 3, pokemon_id: 584}]
        actions.each_with_object(actions_by_priority) do |action, actions_by_priority|
          case action
          in { action: 'attack', player_id:, id:, pokemon_id:, targets: }
            priority = move_priority(id)

            actions_by_priority[priority] << incomming_attack(player_id:, pokemon_id:, id:, targets:)
          in { action: 'attack_switch', player_id:, id:, pokemon_id:, targets:, pokemon_switched_id: }
            priority = move_priority(id)

            actions_by_priority[priority] << incomming_attack_switch(player_id:, pokemon_id:, id:, targets:, pokemon_switched_id:)
      ...
      def incomming_attack(player_id:, pokemon_id:, id:, targets:)
        { player_id:, pokemon_id:, id:, targets: }
      end

      def incomming_attack_switch(player_id:, pokemon_id:, id:, targets:, pokemon_switched_id:)
        { player_id:, pokemon_id:, id:, targets:, pokemon_switched_id: }
      end
