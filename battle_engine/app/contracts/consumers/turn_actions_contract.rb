    ...
    class TurnActionsContract < Dry::Validation::Contract
      params do
        ...
        required(:actions).array(:hash) do
          ...
          optional(:attack_id).maybe(:integer)
      ...
      rule(:actions) do
        ...
        values[:actions].each_with_index do |action_item, index|
          ...
          when 'attack'
            key([:actions, index, :attack_id]).failure('must be filled for attack action') if action_item[:attack_id].nil?
          ...
          when 'attack_switch'
            key([:actions, index, :attack_id]).failure('must be filled for attack_switch action') if action_item[:attack_id].nil?
