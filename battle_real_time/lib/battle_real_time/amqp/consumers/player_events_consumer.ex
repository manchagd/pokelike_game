defmodule BattleRealTime.AMQP.Consumers.PlayerEventsConsumer do
  ...
  alias BattleRealTime.Contracts.Consumers.PlayerEvents.PokemonTemplatesListContract
  alias BattleRealTime.Contracts.Consumers.PlayerEvents.PokemonTemplateMovesListContract
  alias BattleRealTime.Contracts.Consumers.PlayerEvents.TeamsInfoContract
  alias BattleRealTime.Contracts.Consumers.PlayerEvents.TeamDetailsContract
  ...
  def process_message("pokemon_templates_list" = event, data) do
    case PokemonTemplatesListContract.validate(data) do
      {:ok, validated_data} ->
        player_id = validated_data.player_id
        broadcast_player_event(player_id, event, validated_data)
  ...
  def process_message("pokemon_template_moves_list" = event, data) do
    case PokemonTemplateMovesListContract.validate(data) do
      {:ok, validated_data} ->
        player_id = validated_data.player_id
        broadcast_player_event(player_id, event, validated_data)
  ...
  def process_message("teams_info" = event, data) do
    case TeamsInfoContract.validate(data) do
      {:ok, validated_data} ->
        player_id = validated_data.player_id
        broadcast_player_event(player_id, event, validated_data)
  ...
  def process_message("team_details" = event, data) do
    case TeamDetailsContract.validate(data) do
      {:ok, validated_data} ->
        player_id = validated_data.player_id
        broadcast_player_event(player_id, event, validated_data)
